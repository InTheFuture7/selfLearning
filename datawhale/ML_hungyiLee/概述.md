...
步骤：假设当前只有训练集和验证集，将训练集划分成 $k$ 份，取第 $1$ 份作为验证集，取后面 $k-1$ 份作为训练集，模型在 $k-1$ 份数据上训练特定的轮数（epoch），然后在验证集上评估，得到验证集上的损失值。然后取第 $2$ 份作为验证集，取剩下 $k-1$ 作为训练集，得到在这一组数据下的损失值。计算 $k$ 份验证集的平均指标值作为该组参数下模型的指标。然后基于相同的步骤评估其他组参数下模型的指标，直到评估完所有的模型。
...
比如：当训练损失逐渐减小，而测试集的损失值先减小后增大，表明需要增加更多的特征和参数，增加模型复杂度
