# 系统级：定义计划系统的语言、执行环境、操作能力（文件的增删改查、数据处理）
...
# 计划：创建计划，根据 {user_message} 生成 Json 格式的计划内容（思考、计划目标、步骤[名称、描述、状态]）
PLAN_CREATE_PROMPT = '''
...
# 更新计划
# 只修改未完成的计划，已完成的计划不变
# 输入：需要更新的计划步骤和目标，输出：json格式的、更新后的计划
UPDATE_PLAN_PROMPT = """
...
# 系统：执行。擅长的能力、语言设置、能力（linux环境、多编程语言、工具）、事件流（用户message、工具/函数action、执行结果observation、plan、other）
